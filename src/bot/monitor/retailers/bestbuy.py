"""BestBuy.com retailer adapter.

Implements full checkout flow for bestbuy.com using Playwright headless
...
class BestBuyAdapter(RetailerAdapter):
    ...
    # BestBuy API endpoints
    _API_BASE = "https://www.bestbuy.com"
    _CART_URL = "https://www.bestbuy.com/cart"
    _CHECKOUT_URL = "https://www.bestbuy.com/checkout/r/fulfillment"

    # BestBuy product page URL pattern
    _PRODUCT_URL_TEMPLATE = "https://www.bestbuy.com/site/{sku}.p"

    def __init__(self, config: Any) -> None:
        """Initialize the BestBuy adapter.
    ...
    def _log(self, level: str, event: str, **kwargs: Any) -> None:
        ...
        """
        if self._logger:
            getattr(self._logger, level.lower())(event, retailer="bestbuy", **kwargs)
        else:
    ...
    async def login(self, username: str, password: str) -> bool:
        """Authenticate with bestbuy.com using Playwright.

        Args:
            username: BestBuy account email.
            password: BestBuy account password.
        ...
        try:
            ...
            await self._page.fill(email_selector, username, timeout=10000)
    ...
    async def _verify_login_success(self) -> bool:
        ...
        if self._page is None:
            return False
        try:
            # Look for account header/user nav which indicates logged-in state
            await self._page.wait_for_selector(
                '[data-track="Account",data-track-module="Header"], '
                '.account-button, '
            ...
            pass
        # Check URL - should not contain 'signin'
        url = self._page.url.lower()
        return "signin" not in url or "/account/" in url
    ...
    async def _extract_auth_token(self) -> str:
        ...
        for cookie in await self._context.cookies():
            if cookie["name"] in ("t", "token", "sessionId", "auth", "pdsession", "customerInfo"):
                return cookie["value"]
        return ""
    ...
    async def check_stock(self, sku: str) -> StockStatus:
        """Check if a SKU is in stock at BestBuy.com.

        Args:
            sku: BestBuy.com product SKU (numeric, e.g., "12345678").
        ...
        try:
            ...
            product_url = self._PRODUCT_URL_TEMPLATE.format(sku=sku)

            self._log("DEBUG", "STOCK_CHECK", sku=sku, url=product_url)

            # Try BestBuy's public API first (fastest, < 500ms target)
            ...
            try:
                await self._page.wait_for_selector(
                    ".sku-title, .heading-5.v-fw-regular, h1.heading-5",
                    timeout=10000,
                )
            except Exception:  # noqa: BLE001
                pass
    ...
    async def _check_stock_api(self, sku: str) -> StockStatus | None:
        """Query BestBuy's API for stock status (fast path, < 500ms)."""
        try:
            # BestBuy's product availability API
            api_url = (
                f"https://www.bestbuy.com/api/cart/service/v2/items?"
                f"skus={sku}&source=current&responses=1"
            )
            headers = {
                "User-Agent": (
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                # BestBuy API returns an array with item data
                if isinstance(data, list) and len(data) > 0:
                    item = data[0]
                    in_stock = item.get("inStore", False) or item.get("online", False)
    ...
    async def check_stock_by_keyword(self, keyword: str) -> StockStatus:
        """Check for in-stock items matching a keyword at BestBuy.com.

        Navigates to the BestBuy search page for the keyword, collects product
        ...
        try:
            ...
            search_url = (
                f"https://www.bestbuy.com/site/searchpage?query="
                f"{keyword.replace(' ', '+')}"
            )
            ...
            try:
                ...
                for el in link_els[:10]:
                    ...
                    if href and ".p" in href:
                        ...
                        sku_match = self._extract_sku_from_url(href)
                        if sku_match and sku_match not in seen:
                            seen.add(sku_match)
                            full_url = (
                                href
                                if href.startswith("http")
                                else f"https://www.bestbuy.com{href}"
                            )
    ...
    def _extract_sku_from_url(url: str) -> str:
        """Extract SKU from a BestBuy product URL.

        Args:
            url: BestBuy product URL (e.g., https://www.bestbuy.com/site/.../p?id=123456).

        Returns:
            SKU string (e.g., "123456") or empty string if not found.
        """
        import re

        # Pattern: /site/.../sku.p?id=SKU or /site/.../SKU.p
    ...
    async def _parse_stock_from_page(
        ...
        try:
            # BestBuy "Add to Cart" button indicates in-stock
            ...
            try:
                qty_el = await self._page.query_selector(
    ...
    async def add_to_cart(self, sku: str, quantity: int = 1) -> bool:
        ...
        try:
                ...
                if isinstance(retailer_items, list) and retailer_items:
                    for item in retailer_items:
                        item_skus = (
                            ...
                            else getattr(item, "skus", [])
                        )
                        if sku in item_skus:
                            max_qty = (
                                item.get("max_cart_quantity", 1)
                                if isinstance(item, dict)
                                else getattr(item, "max_cart_quantity", 1)
                            )
                            break
            if max_qty == 1:
            ...
            # Try BestBuy quick add API first
    ...
    async def _add_to_cart_api(
        ...
        """Add item via BestBuy's cart API."""
        if not self._auth_token:
            return False

        try:
            # BestBuy cart add API
            api_url = (
                f"https://www.bestbuy.com/cart/api/v2/add"
            )
            headers = {
                ...
                "Referer": self._PRODUCT_URL_TEMPLATE.format(sku=sku),
                "Accept": "application/json",
            }
            payload = {
                "items": [
    ...
    async def _add_to_cart_ui(
        ...
        try:
            ...
            try:
                qty_select = await self._page.query_selector(
                    'select[id="qty"], select[name="qty"]'
                )
                if qty_select:
    ...
    async def _get_cart_api(self) -> list[dict[str, Any]] | None:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                cart_items = data.get("cartItems", []) or data.get("items", [])
    ...
    async def _get_cart_ui(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            items: list[dict[str, Any]] = []
            # BestBuy cart items in list items
            item_rows = await self._page.query_selector_all(
                ".cart-item, .item-row, .cart-row"
            )
            for row in item_rows:
                try:
                    name_el = await row.query_selector(
                        ".product-name, .item-name, h4 a"
                    )
                    qty_el = await row.query_selector(
                        'select[name="quantity"], input[name="quantity"]'
                    )
                    price_el = await row.query_selector(
                        ".price, .item-price, .product-price"
                    )
                    sku_el = await row.query_selector(".sku, [data-sku]")
                    items.append({
                        "sku": (
                        ...
                        "quantity": int(
                            (
                                await qty_el.get_attribute("value")
                                if qty_el
                                else "1"
                            ) or "1"
                        ),
    ...
    async def _fill_shipping(self, shipping: ShippingInfo) -> None:
        """Fill shipping address form fields on BestBuy checkout."""
        if self._page is None:
            return
        ...
        field_mapping: list[tuple[str, str]] = [
            (
                ...
                shipping.name.split()[0] if shipping.name else "",
            ),
            (
                'input[id="lastName"], input[name="lastName"], '
                'input[aria-label*="last" i]',
                " ".join(shipping.name.split()[1:]) if shipping.name else "",
            ),
            (
                'input[id="street"], input[name="street"], '
                'input[id="addressLine1"], input[name="addressLine1"], '
                'input[aria-label*="address" i]',
                shipping.address1,
            ),
            (
                'input[id="street2"], input[name="street2"], '
                'input[id="addressLine2"], input[name="addressLine2"]',
                shipping.address2 or "",
            ),
            (
            ...
            (
                'input[id="zipcode"], input[id="postalCode"], '
        ...
        # Check "billing same as shipping" if present
        try:
    ...
    async def _fill_payment(self, payment: PaymentInfo) -> None:
        """Fill payment card form fields on BestBuy checkout."""
        if self._page is None:
            return

        try:
            # Card number
            card_field = await self._page.query_selector(
                'input[id="cardNumber"], '
        ...
        # CVV
        try:
            cvv_field = await self._page.query_selector(
                'input[id="cvv"], '
                'input[name="cvv"], '
    ...
    async def _handle_review_step(self) -> dict[str, Any]:
        ...
        try:
            ...
            place_order_selectors = [
                ...
                '.checkout-buttons__place-order button',
        ...
        try:
            ...
            try:
                await self._page.wait_for_selector(
                    ...
                    ".order-summary, h1:has-text('Thank')",
                    timeout=30000,
                )
            except Exception:  # noqa: BLE001:
                pass
            ...
            for selector in confirmation_selectors:
                try:
                    ...
                    if el:
                        ...
                        # BestBuy order numbers are typically 8+ digits
            ...
            try:
                total_el = await self._page.query_selector(
                    ".order-total, .grand-total, .total-price"
                )
                if total_el:
    ...
    async def handle_captcha(self, page: Any) -> CaptchaSolveResult:
        ...
        BestBuy primarily uses Cloudflare Turnstile, which should be
        auto-solved in smart mode.
        ...
        try:
            ...
            if captcha_mode == "manual":
                ...
                # BestBuy uses Turnstile — auto-solve even in smart mode
    ...
    async def _detect_captcha_type(self, page: Any) -> CaptchaType:
        ...
        try:
            ...
            # Check for Turnstile (Cloudflare — primary for BestBuy)
            try:
                await page.wait_for_selector(
            ...
            # Also check for Cloudflare challenge page redirect
            try:
                cloudflare_challenge = await page.query_selector(
                    "#challenge-form, #cf-challenge-running, "
                    '.cf-browser-verification, #turnstile-wrapper'
                )
                if cloudflare_challenge and await cloudflare_challenge.is_visible():
                    return CaptchaType.TURNSTILE
            except Exception:  # noqa: BLE001:
                pass
    ...
    async def _extract_site_key(
        ...
        """Extract CAPTCHA site key from BestBuy page."""
        try:
            # Turnstile typically uses data-sitekey attribute or appears in page content
            selectors = [
                '[data-sitekey]',
                '.cf-turnstile',
                '#turnstile-wrapper',
            ...
            # Also try page content regex for Turnstile sitekey
            ...
            if captcha_type == CaptchaType.TURNSTILE:
                ...
                if match:
                    ...
                    if g1 is not None:
                        return g1
                # Alternative pattern used by Cloudflare
                pattern2 = r'["\']sitekey["\']\s*:\s*["\']([A-Za-z0-9_-]+)["\']'
                match2 = re.search(pattern2, page_content)
                if match2:
                    g2: str | None = match2.group(1)
                    if g2 is not None:
                        return g2
            ...
            if captcha_type == CaptchaType.RECAPTCHA_V2:
                ...
                await page.evaluate(
                    ...
                    document.querySelectorAll('[name="cf-turnstile-response"]')
                        .forEach(el => el.value = "{token}");
                    // Also try to trigger Cloudflare's callback
                    if (typeof ___cf_challenge_done === 'function')
                        ___cf_challenge_done("{token}");
                    if (typeofturnstileCallback === 'function')
                        turnstileCallback("{token}");
                    // Dispatch a custom event that Cloudflare listens for
    ...
    async def check_queue(self) -> bool:
        ...
        try:
            ...
            # Check for BestBuy-specific queue/waiting room elements
            try:
                queue_el = await self._page.query_selector(
                    ".queue-container, .waiting-room, "
                    ".virtual-line, .queue-page, #queue"
                )
                if queue_el and await queue_el.is_visible():
                    return True
            except Exception:  # noqa: BLE001:
                pass
