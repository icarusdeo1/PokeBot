"""Target.com retailer adapter.

Implements full checkout flow for target.com using Playwright headless
...
class TargetAdapter(RetailerAdapter):
    ...
    name: str = "target"
    base_url: str = "https://www.target.com"

    # API endpoints
    CART_API_URL = "https://redsky.target.com/redsky/general/v1/cart"
    CART_ADD_URL = "https://redsky.target.com/redsky/general/v1/cart/add"
    ...
    def _log(self, level: str, event: str, **kwargs: Any) -> None:
        ...
        """
        if self._logger:
            getattr(self._logger, level.lower())(event, retailer="target", **kwargs)
        else:
    ...
    async def login(self, username: str, password: str) -> bool:
        """Authenticate with target.com using Playwright.

        Args:
            username: Target.com account email.
            password: Target.com account password.
        ...
        try:
            ...
            await self._page.goto(
                "https://www.target.com/co/login",
                wait_until="networkidle",
                timeout=30000,
            )

            # Fill login form
            await self._page.fill('input[name="username"]', username, timeout=10000)
            await asyncio.sleep(random.uniform(0.3, 0.7))

            # Handle "Continue" button if present
            continue_btn = self._page.locator('button[type="submit"]')
            ...
            # Fill password
            await self._page.fill('input[name="password"]', password, timeout=10000)
    ...
    async def _verify_login_success(self) -> bool:
        ...
        try:
            await self._page.wait_for_selector(
                '[data-test="@web/AccountHeader"] a[href="/account"]',
                timeout=5000,
            )
            return True
    ...
    async def _extract_auth_token(self) -> str:
        """Extract authentication token from page or cookies."""
        if self._context is None:
            return ""
        for cookie in await self._context.cookies():
            if cookie["name"] in ("tc", "token", "auth", "sessionId"):
                return cookie["value"]
        return ""
    ...
    async def _save_cookies(self) -> None:
        ...
        # Store in session state via base class
    ...
    async def check_stock(self, sku: str) -> StockStatus:
        """Check if a SKU is in stock at Target.com.

        Args:
            sku: Target.com product SKU (e.g., "83821795").
        ...
        try:
            ...
            # Try direct product API first (faster)
    ...
    async def _check_stock_api(self, sku: str) -> StockStatus | None:
        """Query Target's internal API for stock status."""
        try:
            api_url = (
                f"https://redsky.target.com/redsky/general/v1/product/"
                f"{sku}?pricing_buying_mode=BUYING_MODE"
                f"&pricing_store_id=DEFAULT"
            )
            headers = {
                "User-Agent": (
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                location_availability = product.get("location_availability", [])

                in_stock = False
                available_qty = 0
                if location_availability:
                    loc = location_availability[0]
                    in_stock = loc.get("availability_code") in (
                        "IN_STOCK",
                        "LIMITED_STOCK",
                    )
                    available_qty = loc.get("quantity", 0)
        ...
        try:
            ...
            if not in_stock:
                stock_text_selectors = [
                    ...
                    'p:has-text("Out of Stock")',
                ]
                for selector in stock_text_selectors:
                    try:
                        el = await self._page.wait_for_selector(
                            selector,
                            timeout=2000,
            ...
            try:
                qty_selector = '[data-test="product-quantity"] input'
                qty_el = await self._page.query_selector(qty_selector)
                if qty_el:
    ...
    async def add_to_cart(self, sku: str, quantity: int = 1) -> bool:
        ...
        try:
            ...
            # Use retailer-specific items if available and properly structured
                ...
                if isinstance(retailer_items, list) and retailer_items:
                    for item in retailer_items:
                        ...
                        if sku in item_skus:
                            ...
                            break
            # Fallback: check top-level config.items (legacy/backward-compat)
            if max_qty == 1:
    ...
    async def _add_to_cart_api(
        ...
        """Add item via Target's internal cart API."""
        ...
        try:
            ...
            try:
                qty_select = await self._page.query_selector(
                    '[data-test="product-quantity"] select, '
    ...
    async def _get_cart_api(self) -> list[dict[str, Any]] | None:
        """Fetch cart via Target's internal API."""
        if not self._auth_token:
            return None
        try:
            headers = {
            ...
            timeout = httpx.Timeout(10.0)
            async with httpx.AsyncClient(timeout=timeout) as client:
                resp = await client.get(self.CART_API_URL, headers=headers)
                if resp.status_code != 200:
                    return None
    ...
    async def _get_cart_ui(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for row in item_rows:
                try:
                    sku_el = await row.query_selector('[data-test="item-sku"]')
                    name_el = await row.query_selector('[data-test="item-title"]')
                    qty_el = await row.query_selector('[data-test="item-quantity"]')
                    price_el = await row.query_selector('[data-test="item-price"]')
                    items.append({
                        "sku": await sku_el.get_attribute("value") if sku_el else "",
    ...
    async def _handle_checkout_page(
        ...
        try:
            # Check for 1-Click option first
            use_1click = getattr(
                self.config.checkout,
                "use_1click_if_available",
                True,
            )
            try:
                click_1_btn = await self._page.wait_for_selector(
                    'button:has-text("1-Click checkout")',
                    timeout=3000,
                )
                if click_1_btn and use_1click:
                    await click_1_btn.click()
                    self._log("INFO", "CHECKOUT_USING_1CLICK")
                    await asyncio.sleep(2.0)
    ...
    async def _fill_shipping(self, shipping: ShippingInfo) -> None:
        ...
        field_mapping: list[tuple[str, str]] = [
            ('input[name="shipping-name"]', shipping.name),
            ('input[name="shipping-address1"]', shipping.address1),
            ('input[name="shipping-address2"]', shipping.address2),
            ('input[name="shipping-city"]', shipping.city),
            ('select[name="shipping-state"]', shipping.state),
            ('input[name="shipping-zip"]', shipping.zip_code),
            ('input[name="shipping-phone"]', shipping.phone),
            ('input[name="shipping-email"]', shipping.email),
        ]
        ...
        try:
            same_as_shipping = await self._page.query_selector(
                'input[name="billingSameAsShipping"]'
            )
            if same_as_shipping:
    ...
    async def _fill_payment(self, payment: PaymentInfo) -> None:
        ...
        for mm_sel, yy_sel in [
            ('select[name="expiry-month"]', 'select[name="expiry-year"]'),
            ('select[name="card-exp-month"]', 'select[name="card-exp-year"]'),
        ]:
            try:
    ...
    async def _handle_review_step(self) -> dict[str, Any]:
        ...
        try:
            terms_selectors = [
                'input[name="agreedToTerms"]',
                'input[data-test="terms-checkbox"]',
            ]
